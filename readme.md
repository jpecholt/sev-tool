...
6. pdh_cert_export
     ...
     - Example
         ...
         ```
7. pek_cert_import
This command imports the signed PEK CSR and OCA.cert which were previously created and then provided by the user.
     - Required input args:
...
14. calc_measurement
     ...
     - Required input args: Note the format of the input parameters are ascii-encoded hex bytes.
         ...
         - [MNonce]: Provided by PO in some way. During VM launch, Launch_measure creates the nonce and appends it to the measure it calculated, see command description in the [API](https://www.amd.com/system/files/TechDocs/55766_SEV-KM_API_Specification.pdf). Obtain  MNonce by separating measure from nonce. See [qmp](https://www.qemu.org/docs/master/interop/qemu-qmp-ref.html) and `query-sev-launch-measure` to obtain the launch measure. Note the value is in base64 format.
...
16. generate_launch_blob
     ...
     - Example
         ...
         ```
17. package_secret
     - This command reads in the pek.cert for API information, the file generated by generate_launch_blob (tmp_tk.bin) for the TEK and the secret file (secret.txt) which is to be encrypted/wrapped by the TEK. It then outputs a file (packaged_secret.txt) which can be passed into Launch_Secret as part of the normal API flow
